...
* ボットを有効化し、チャンネルまたはグループに追加します。

* 目的の会話でメッセージを送信します（内容は任意）。

* 「最新の会話IDを取得」をクリックし、監視してメッセージを受信したい会話を選択します。

* [オプション] トピック機能を有効にした Telegram グループを使用している場合は、`トピック ID` 欄に入力すると、ボットが正しいトピックにメッセージを送信します。
...
>* 電話番号を非表示 - プライバシー保護のため、転送されるメッセージ内の電話番号を非表示にします。
>* DNS over HTTPSの使用 - 暗号化された安全なDNSサービスを使用（Cloudflare®基盤）。
>* カーボンコピー - Bark、Lark、PushDeerなどの他のサービスに通知を転送します。複数のサービスを設定してメッセージのコピーを受け取ることができます。
