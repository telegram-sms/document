...
В отличие от прямого многострочного формата `/sendsms`, описанного выше, отправка `/sendsms` без аргументов запускает интерактивный пошаговый процесс. Бот запрашивает каждую часть информации по очереди с помощью ForceReply в Telegram (поле ввода ответа сразу активно), поэтому вам нужно просто отвечать на каждый запрос:

1. На устройствах с двумя SIM-картами (если номер SIM не указан в команде) бот сначала показывает встроенную клавиатуру для выбора «SIM 1» или «SIM 2». На устройствах с одной SIM-картой этот шаг пропускается.
2. Затем бот запрашивает номер телефона получателя — вы отвечаете номером.
3. Далее бот запрашивает текст сообщения — вы отвечаете текстом.
4. Бот отправляет SMS через выбранную SIM-карту и подтверждает.
