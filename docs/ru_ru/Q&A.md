...
В Telegram SMS есть пункт **Указать адрес API** в дополнительном меню (⋮) приложения. Он предназначен для того, чтобы направить приложение на ваш собственный [самостоятельно размещённый сервер Telegram Bot API](https://dev.telegram-sms.com/self_hosted_bot_api).

**Переключайтесь только на сервер, который вы полностью контролируете и которому доверяете. Направление приложения на недоверенный сервер API аналогично перехвату GSM + sniffing SMS и может привести к финансовым потерям — все ваши сообщения и токен бота проходят через сервер, который вы укажете.**
