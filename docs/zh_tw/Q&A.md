...
Telegram SMS 已在應用的溢出選單（⋮）中提供 **設置 API 地址** 選項，用於將應用指向您[自建的 Telegram Bot API 服務器](https://dev.telegram-sms.com/self_hosted_bot_api)。

**請僅切換到您完全掌控並信任的服務器。將應用指向不受信任的 API 服務器如同 GSM劫持 + 短信嗅探，有可能導致您的財產損失——您的每一條消息和機器人令牌都會經過您所配置的服務器。**
