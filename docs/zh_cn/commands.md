...
与上文介绍的直接多行 `/sendsms` 格式不同，直接发送不带参数的 `/sendsms` 会启动交互式的分步流程。机器人会使用 Telegram 的 ForceReply（回复框会自动聚焦）逐项提示您输入信息，您只需逐条回复即可：

1. 在双卡设备上（命令未指定 SIM 卡编号时），机器人会先显示一个内联键盘供您选择「SIM 1」或「SIM 2」。单卡设备会跳过此步骤。
2. 接着机器人会询问收件人电话号码——您回复号码即可。
3. 然后机器人会询问短信内容——您回复文本即可。
4. 机器人通过所选的 SIM 卡发送短信并确认。
