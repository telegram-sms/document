...
Telegram SMS 已在应用的溢出菜单（⋮）中提供 **设置 API 地址** 选项，用于将应用指向您[自建的 Telegram Bot API 服务器](https://dev.telegram-sms.com/self_hosted_bot_api)。

**请仅切换到您完全掌控并信任的服务器。将应用指向不受信任的 API 服务器如同 GSM劫持 + 短信嗅探，有可能导致您的财产损失——您的每一条消息和机器人令牌都会经过您所配置的服务器。** 
